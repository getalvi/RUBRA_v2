// frontend/src/components/Message.jsx — REPLACE ENTIRE FILE

import { useState, useRef, useEffect } from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import {
  Pencil, Check, X, Copy, CheckCheck, Hexagon, Cpu, Brain,
  Search, FileText, GraduationCap, MessageCircle, PanelRight
} from 'lucide-react'
import ReactMarkdown from 'react-markdown'
import remarkGfm from 'remark-gfm'

// ── Code block with copy + "Open in Panel" ───────────────
function CodeBlock({ children, className, onOpenPanel, artifactId }) {
  ...
  // Is this block substantial enough to show panel button?
  const isSubstantial = code.split('\n').length >= 4 || code.length >= 200
  const isPanelable   = isSubstantial && !['bash','sh','shell','cmd','text','txt','markdown','md'].includes(lang)
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          {isPanelable && onOpenPanel && (
            <button
              onClick={() => onOpenPanel(artifactId)}
      ...
      {/* Code */}
      <pre
        className="px-4 py-3.5 overflow-x-auto text-[13px] leading-relaxed font-mono m-0 border-0"
...
// ── Markdown ─────────────────────────────────────────────
function Markdown({ text, artifacts, onOpenPanel }) {
  const clean = text
  ...
  // Build a map of code→artifactId for matching blocks to panel entries
  const codeArtifactMap = {}
  if (artifacts) {
    for (const a of artifacts) {
      codeArtifactMap[a.code.slice(0, 80)] = a.id
    }
  }
      ...
      <ReactMarkdown
        ...
        components={{
          code({ inline, className, children }) {
            ...
            const artifactId = codeArtifactMap[key] || null
            codeBlockIndex++
            return (
              <CodeBlock
                className={className}
          ...
          a: ({ href, children }) => (
            <a
              ...
              target="_blank"
              rel="noopener"
              style={{ color: '#fb7185', textDecoration: 'underline', textUnderlineOffset: 2 }}
            >
              {children}
            </a>
          ),
...
// ── Assistant message ─────────────────────────────────────
function AssistantMsg({ msg, isStreaming, artifacts, onOpenPanel }) {
  const [copied, setCopied] = useState(false)
...
// ── Export ────────────────────────────────────────────────
export default function Message({ msg, isStreaming, onEdit, artifacts, onOpenPanel }) {
